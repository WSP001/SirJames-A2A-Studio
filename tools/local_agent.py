...
class LocalAgent:
    ...
    def validate(self, chapter=None):
        ...
        if chapter:
            ...
            if chapter_dir.exists():
                print(f"  ✅ Chapter {chapter} directory exists")
                # Check for scene files
                scene_files = list(chapter_dir.glob("scene-*/index.html"))
                print(f"  ✅ Found {len(scene_files)} scene files")
            else:
                print(f"  ❌ Chapter {chapter} directory missing")
        else:
            print("  ℹ️ Specify --chapter N to validate specific chapter")
        ...
        print("\n" + "-" * 60)
        if all_good:
            print("✅ VALIDATION PASSED - Ready for Book002 generation!")
        else:
    ...
    def test_connections(self):
        ...
        try:
            ...
            if api_key:
                ...
                print(f"  ✅ OpenAI: Connected ({len(list(response))} models available)")
            else:
        ...
        if api_key:
            ...
            print("  ℹ️ Voice synthesis test requires actual generation")
        else:
            print("  ⚠️ ElevenLabs: API key not set (optional for Phase 1)")
        
        # Test Suno (if available)
        print("\n[4/4] Testing Suno API...")
        api_key = os.environ.get('SUNO_API_KEY')
        if api_key:
            print("  ✅ Suno: API key configured")
            print("  ℹ️ Music generation test requires actual generation")
        else:
            print("  ⚠️ Suno: API key not set (optional for Phase 1)")
    ...
    def generate_chapter(self, chapter, images_only=False):
        ...
        if not (1 <= chapter <= 10):
            print(f"❌ Invalid chapter number: {chapter} (must be 1-10)")
            return False
        ...
        if not orchestrate_script.exists():
            print(f"❌ Orchestration script not found: {orchestrate_script}")
            return False
    ...
    def show_status(self):
        ...
        if status_file.exists():
            ...
            print(f"  📄 Status updated: {modified.strftime('%Y-%m-%d %H:%M')}")
    ...
    def prepare_deploy(self):
        ...
        if public_dir.exists():
            ...
            print(f"  ✅ {total_files} files in public-book002/")
        else:
...
def main():
    ...
    if args.command == 'help' or args.command == 'setup':
        ...
        if not args.chapter:
            print("❌ --chapter required for generate command")
            sys.exit(1)
